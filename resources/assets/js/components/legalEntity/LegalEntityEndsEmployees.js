...
class LegalEntityEndsEmployees extends React.Component {
    ...
    render() {
        const {legalEntity, employeesResearchesEnds} = this.props;
        ...
        return (
            <div className="animated fadeIn">
                <Row>
                    <Col xs="12" lg="12">
                        <EmployeesList
                            employees = {employeesResearchesEnds}
                            handleBtnDelete = {this.handleBtnDelete.bind(this)}
                            title = {`Сотрудники «${legalEntity.name}» с заканчивающимся медицинским осмотром `}
                        />
                    </Col>
                </Row>
            </div>
